...
const UploadPage = () => {
  ...
  const [outlinePresets, setOutlinePresets] = useState<
  ...
  // Load saved outline presets when a JSON (InDesign) file is selected.
  const hasJsonFile = files.some((f) => f.name.toLowerCase().endsWith(".json"));
  useEffect(() => {
    if (!hasJsonFile) return;
    let cancelled = false;
    PresentationGenerationApi.listOutlinePresets()
      .then((presets: { id: string; name: string }[]) => {
        if (!cancelled) setOutlinePresets(presets || []);
      })
      .catch(() => {
        if (!cancelled) setOutlinePresets([]);
      });
    return () => {
      cancelled = true;
    };
  ...
  const handleDocumentProcessing = async () => {
    // InDesign JSON exports go through the chat-based outline extraction flow
    // instead of text decomposition. If any uploaded file is .json, route there.
    if (hasJsonFile) {
      await handleIndesignJsonProcessing();
      return;
    }
  ...
  /**
   * Handles an uploaded InDesign JSON export: upload the file, create a
   * presentation referencing it, then route to the chat-based outline
   * extraction page (no text decomposition).
   */
  const handleIndesignJsonProcessing = async () => {
    setLoadingState({
      isLoading: true,
      message: "Uploading InDesign document...",
      showProgress: true,
      duration: 20,
    });
    ...
    const presetQuery = selectedPresetId
      ? `?preset=${encodeURIComponent(selectedPresetId)}`
      : "";
    router.push(`/outline-extract${presetQuery}`);
  };
  ...
  return (
    <Wrapper className="pb-10 lg:max-w-[65%] xl:max-w-[60%]">
      ...
      <div className="rounded-2xl " >
        ...
        <div className="p-4 ">
          <h3 className="text-sm font-medium text-[#333333] mb-2">첨부 파일 (선택)</h3>
          <SupportingDoc
            files={[...files]}
          ...
          {hasJsonFile && (
            <div className="mt-3">
              ...
              <select
                ...
                className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#5146E5]/40"
              >
                <option value="">프리셋 없음 (새로 교정)</option>
                {outlinePresets.map((p) => (
              ...
              <p className="text-xs text-gray-500 mt-1">
                이전 단원에서 저장한 교정 규칙을 이 InDesign 문서에 적용합니다.
              </p>
            </div>
          )}
        </div>
